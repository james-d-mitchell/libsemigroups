  ...
  namespace fpsemigroup {
    ...
    LIBSEMIGROUPS_TEST_CASE("Kambites",
      ...
      for (auto const& w : words) {
        std::string ww = k.normal_form(w);
        if (!k.equal_to(ww, w)) {
          std::cout << "original word = " << std::endl << w << std::endl;
          std::cout << "normal form = " << std::endl << ww << std::endl;
          REQUIRE(std::string(w.cbegin(),
                              detail::maximum_common_prefix(
                                  w.cbegin(), w.cend(), ww.cbegin(), ww.cend())
                                  .first)
                  == std::string(""));
          // REQUIRE(std::equal(ww.cbegin(), ww.cend(), w.cbegin(), w.cend()));
        }
      }
    }
