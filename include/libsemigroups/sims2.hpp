...
namespace libsemigroups {
  ...
  class Sims2Graph {
   ...
   public:
    ...
    digraph_type const &left() const {
      return _left;
    }

    digraph_type const &right() const {
      return _right;
    }
    ...
    void validate() const noexcept {
      ...
      LIBSEMIGROUPS_ASSERT(
          ...
          // The previous happens if the graph is complete, then
          // the first undefined edge can be for incident to a
          // larger node, whose length isn't defined
     ...
     public:
      Sims2(Presentation<word_type> const &p)
        ...
        for (auto it = p.cbegin(); it != p.cend(); it += 2) {
          _left_presentation.add_rule(it->crbegin(),
                                      it->crend(),
                                      (it + 1)->crbegin(),
                                      (it + 1)->crend());
        }
      ...
      class const_iterator {
       public:
        ...
        using difference_type =
            typename std::vector<Sims2Graph>::difference_type;
        //! No doc
        ...
        using const_reference =
            typename std::vector<Sims2Graph>::const_reference;
        //! No doc
          ...
          dive:
            ...
            for (node_type next = current.source; next < _num_active_nodes;
              ...
              for (; a < _word_graphs.right().out_degree(); ++a) {
                if (_word_graphs.right().unsafe_neighbor(next, a)
                    == UNDEFINED) {
