...
namespace libsemigroups {
  ...
  namespace {
    ...
    paths_in_lex_order(WordGraph<T> const& wg,
                       S const             root,
                       size_t              min = 0,
      ...
      while (!node.empty()) {
        std::tie(edge, next)
            = wg.next_label_and_target_no_checks(node.back(), edge);
    ...
    paths_in_lex_order2(WordGraph<T> const& wg,
                        S const             first,
                        S const             last,
                        size_t              min = 0,
                        size_t max = libsemigroups::POSITIVE_INFINITY) {
    ...
    template <typename T, typename S>
    std::pair<std::vector<word_type>, std::vector<node_type<T>>>
    paths_in_shortlex_order(WordGraph<T> const& wg,
                            S const             root,
                            size_t              min = 0,
                            size_t max = libsemigroups::POSITIVE_INFINITY) {
      using node_type  = node_type<T>;
      ...
      auto out = paths_in_lex_order(wg, root, min, std::min(min + 1, max));

      for (size_t i = 0; i < out.first.size() && out.first[i].size() < max;
           ++i) {
        node_type  n;
        label_type a   = 0;
        std::tie(a, n) = wg.next_label_and_target_no_checks(out.second[i], a);
        while (n != UNDEFINED) {
          ...
          std::tie(a, n)
              = wg.next_label_and_target_no_checks(out.second[i], ++a);
        }
      }
      return out;
    }

    WordGraph<size_t> test_word_graph() {
      WordGraph<size_t> wg;
      wg.add_nodes(6);
  ...
  TEST_CASE("const_pislo_iterator x 1", "[quick][004][pislo]") {
    using node_type = size_t;
    auto   wg       = test_word_graph();
    size_t N        = 19;
