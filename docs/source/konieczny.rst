...
``libsemigroups`` implements %Konieczny's algorithm for computing the structure
of finite semigroups that act on sets; see the documentation below for further
details.

The implementations of this algorithm is generic, and can be easily adapted
...
The implementation of %Konieczny's algorithm can be used "interactively", in the
sense that they can be run for a particular amount of time, or until some
condition is met; for further details see, for example, :cpp:any:`void
libsemigroups::Runner::run_for(std::chrono::nanoseconds)` and
:cpp:func:`libsemigroups::Runner::run_until`.

The classes implementing %Konieczny's algorithm are:
...
.. toctree::
   ...
   _generated/libsemigroups__konieczny__basedclass
